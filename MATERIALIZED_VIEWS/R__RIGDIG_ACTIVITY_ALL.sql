create or replace materialized view ITD_SCHEMACHANGE_DEMO.RIGDIG_ACTIVITY_ALL
...
select SPLIT_PART(metadata$filename,'/',-1)::VARCHAR(250) AS FILE_NAME
      ...
      ,NULLIF(value:c9,'')::VARCHAR(150) AS ACT_LOCATION_DESC
      ...
      ,NULLIF(value:c15,'')::VARCHAR(150) AS ACT_INSP_VIOL_CITED
      ,NULLIF(value:c16,'')::VARCHAR(150) AS ACT_INSP_VIOL
